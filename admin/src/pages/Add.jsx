...
const Add = ({token}) => {
  ...
  try {
   ...
   const response = await axios.post(backend_url + "/api/product/add",formData,{headers:{token:token}});
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col items-start w-full gap-3'>
      ...
      <div className='w-full flex flex-col sm:flex-row gap-2 sm:gap-8'>
        ...
        <div>
          ...
          <input onChange={(e)=>setPrice(e.target.value)} className='w-full px-3 py-2 sm:w-[125px]' type="number" placeholder='25'/>
        </div>
      </div>
      <div>
      ...
      <button type="submit" className='w-28 py-2 bg-black text-white mt-4'>ADD</button>
    </form>
  )
}
